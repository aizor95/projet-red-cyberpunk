...
func explore(p *Character) {
	...
	for {
		...
		for i, action := range actions {
			printOption(i+1, action)
		}
		printPanel("Deplacement")
		for i, connection := range location.Connections {
			...
			if canAccessLocation(
				...
				printOption(i+1+len(actions), "Aller vers "+connection)
			} else {
				fmt.Printf(
					"  %d. %s [bloque - badge %d requis]\n",
					i+1+len(actions),
					connection,
		...
		fmt.Println("  0. Retour au tableau de bord")
		var choice int
		askChoice()
		fmt.Scanln(&choice)
		if choice == 0 {
